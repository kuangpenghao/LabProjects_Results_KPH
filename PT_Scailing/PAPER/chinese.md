...
**Methods.** 本实验基于宽度为 $\text{dim}_z=1536$ 的 PT 模型展开。设零样本迁移所得的超参数集合为中心基准点 $S=\{S_1, S_2, \cdots, S_7\}$。针对任意 $S_i\ (1 \leq i \leq 7)$，我们在其 $\pm 20\%$ 的均匀分布区间 $[0.8S_i, 1.2S_i]$ 内进行独立随机采样，从而构建出中心基准点 $S$ 局部邻域内的扰动采样集合 $S'$。为了以 95% 的置信度（即 $\alpha=0.05$）验证 $S$ 是否落入局部空间内 Top 5%（即 $p=0.05$）的最优区间，根据上述定理可计算出理论最小采样次数 $n \geq 58.4$。为此，我们执行 $n=60$ 次独立的局部随机采样训练，并将其最终的Evaluation Loss与基准点 $S$ 的Evaluation Loss进行量化对比。

**Results.** 实验结果呈现出极其显著的Basin of local minimum效应。在全部 60 组扰动采样中，绝大多数配置的Evaluation Loss相较于zero shot配置出现了上升。尽管存在少量表现最佳的采样点，但其损失相较于 $S$ 仅发生了最多 0.8% 的微幅下降。考虑到优化过程中的Stochastic Noise，这种极其微弱的性能波动处于训练的容差范围之内，在统计意义上可视为与 $S$ 等效。为了直观量化这一现象，我们定义扰动采样点 $S'$ 与基准点 $S$ 在相对参数空间中的距离为 $D(S',S)=\sqrt{\sum_{i=1}^7 (\frac{S_i'-S_i}{S_i})^2}$。图 2 绘制了距离 $D(S',S)$ 与采样点Evaluation Loss上升百分比的散点关系，证实了 $S$ 稳固地处于该陡峭优化盆地的底部。因此，在统计学上，我们有 95% 的极大把握认定，通过零样本迁移获取的超参数配置处于该邻域空间内前 5% 的最优区域。
